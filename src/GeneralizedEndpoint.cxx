...
float GeneralizedEndpoint::GeneralizedEndpointPt(float MuonPt, int MuonCharge, float MuonEta, float MuonPhi, int Mode, int verbose){
  ...
  // Resolution uncertainty to be applied on top to the systematic variations
  // double res = 0;
  // if (MuonEta<1.6) {
  //   if (MuonPt<200) res = 0.3;
  //   else if (MuonPt<500) res = 0.5;
  //   else res = 1.;
  // } else{
  //   if (MuonPt<200) res = 0.6;
  //   else if (MuonPt<500) res = 1.0;
  //   else res = 2.;
  // }
  //
  // if (Mode!=0) KappaBiasError = TMath::Sqrt(KappaBiasError*KappaBiasError+res*res);

  float rnd = KappaBias+99*KappaBiasError;
  while (abs(KappaBias-rnd) > KappaBiasError) rnd = gRandom->Gaus(KappaBias,KappaBiasError);
