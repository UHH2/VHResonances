...
#include "/nfs/dust/cms/user/amalara/WorkingArea/UHH2_102X_v1/CMSSW_10_2_10/src/UHH2/HiggsToWWTagger/include/NeuralNetworkBase.hpp"
...
class SFModule: public ModuleBASE {

public:

  explicit SFModule(uhh2::Context&);
...
void SFModule::book_handles(uhh2::Context& ctx) {
  string tag;
...
void SFModule::book_histograms(uhh2::Context& ctx){
...
SFModule::SFModule(uhh2::Context& ctx){
  ...
  if (MB["isPuppi"] == MB["isCHS"] && MB["isPuppi"] == MB["isHOTVR"]) throw std::runtime_error("In SFModule.cxx: Choose exactly one jet collection.");
  if (MB["muonchannel"] == MB["electronchannel"]) throw std::runtime_error("In SFModule.cxx: Choose exactly one lepton channel.");
...
bool SFModule::process(uhh2::Event& event) {
...
// make sure the SFModule is found by class name. This is ensured by this macro:
UHH2_REGISTER_ANALYSIS_MODULE(SFModule)
