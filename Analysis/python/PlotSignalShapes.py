...
class PlotSignalShapes(VariablesBase):
                ...
                for collection in self.Collections:
                    ...
                    with open(fname, 'r') as f_:
                        ...
                        for sample in self.Samples:
                            for sys in self.Syst:
                                ...
                                # print name
                                self.funcs[name] = ROOT.TF1(name, ExpGaussExp if "inv" in channel else CrystalBall, f_xmin, f_xmax, 5)
                        for line in lines:
                    ...
                    for sample in self.Samples:
                        for sys in self.Syst:
                            ...
                            f_ = ROOT.TF1("", ExpGaussExpIntegral if "inv" in channel else CrystalBallIntegral, f_xmin,f_xmax, 4)
                            f_.SetParameters(func.GetParameter(0),func.GetParameter(1),func.GetParameter(2),func.GetParameter(3))
                            # print name, fname, func.GetParameter(4),
                            fNorms[fname] = 1./f_.Integral(f_xmin,f_xmax)
                            # print func.Integral(f_xmin,f_xmax), f_.Integral(f_xmin,f_xmax)
    ...
    def PlotParameters(self):
        for par in range(0,6):
            # if par == 0: ymin, ymax, ParName, funcName = (0,2,    "#mu [GeV]",    "pol1")
            # if par == 1: ymin, ymax, ParName, funcName = (0,0.2,  "#sigma [GeV]", "pol2")
                ...
                for year in ["RunII"]:
                    for collection in self.Collections:
                        ...
                        for sample in self.Samples:
                            ...
                            y_down.append(down)
                        # if par <=1:
                        #  y_bins = np.array(y_bins)/np.array(x_bins)
