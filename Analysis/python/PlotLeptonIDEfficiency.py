...
colors = {'ele_ID_kincut':                  (ROOT.kBlack,ROOT.kDot),
          'ele_ID_HEEP':                    (ROOT.kAzure+7,ROOT.kOpenSquare),
          'ele_ID_veto':                    (ROOT.kOrange+1,ROOT.kFullCross),
          'ele_ID_loose':                   (ROOT.kGreen+2,ROOT.kFullSquare),
          'ele_ID_medium':                  (ROOT.kRed+1,ROOT.kFullCircle),
          'ele_ID_tight':                   (ROOT.kOrange,ROOT.kDot),
          'ele_ID_MVA_loose_noIso':         (ROOT.kAzure+10,ROOT.kFullTriangleUp),
          'ele_ID_MVA_wp90_noIso':          (ROOT.kAzure+7,ROOT.kFullDiamond),
          'ele_ID_MVA_wp80_noIso':          (ROOT.kBlue+1,ROOT.kFullTriangleDown),
          'ele_ID_MVA_loose_iso':           (ROOT.kGray,ROOT.kFullDiamond),
          'ele_ID_MVA_wp90_iso':            (ROOT.kGreen+1,ROOT.kDot),
          'ele_ID_MVA_wp80_iso':            (ROOT.kMagenta,ROOT.kDot),
...
class PlotLeptonIDEfficiency(VariablesBase):
    def __init__(self,year='2016', collection='Puppi', channel='muonchannel', isDR=True):
        ...
        self.year       = year
        TDR.cms_lumi    = str(round(float(self.lumi_map[year]['lumi_fb']),1))+' fb^{-1}' if TDR.extraText!='Simulation' else year
        ...
        self.IDs        = list(filter(lambda x: not '_iso' in x, self.IDs))
        self.IDs        = list(reversed(sorted(self.IDs)))
        self.MinSetIDs  = ['muon_ID_CutBasedIdGlobalHighPt', 'muon_ID_CutBasedIdGlobalHighPt_2IDs', 'muon_ID_CutBasedIdTrkHighPt', 'muon_ID_CutBasedIdLoose_2IDs', 'muon_ID_CutBasedIdLoose', 'muon_ID_MvaLoose']
        self.MinSetIDs += ['ele_ID_MVA_loose_noIso', 'ele_ID_veto', 'ele_ID_MVA_wp90_noIso', 'ele_ID_loose', 'ele_ID_MVA_wp80_noIso', 'ele_ID_medium']
    ...
    def ResetCanvas(self, name):
        ...
        self.leg = tdrLeg(0.23,0.15,0.95,0.40, 0.045, 42, ROOT.kBlack)
